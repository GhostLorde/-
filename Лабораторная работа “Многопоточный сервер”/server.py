...
def read_file():
    ...
    with open("users.txt", mode="r") as u:
        file = u.readlines()
...
def write_in_file():
    with open("users.txt", mode="w") as u:
...
class multiServer(Thread):
    ...
    def run(self):
        ...
        while (True):
            ...
            if (data == "exit"):
                break
            logging.info(f"Message from user: {self.msg_name}: {data}")
            print(f"Message from user: {self.msg_name}: {data}")
